...
@torch.no_grad()
def test(hn, hf, dataset, chunk_size=10, img_index=0, nb_bins=192, H=400, W=400, epoch=0):
    """
    Args:
        ...
        epoch (int, optional): epoch number for saving the image. Defaults to 0.
    ...
    plt.figure()
    plt.imshow(img)
    plt.savefig(f'novel_views/{epoch}/image_{img_index}', bbox_inches='tight')
    plt.close()
...
def train(nerf_model, optimizer, scheduler, data_loader, device='cpu', hn=0, hf=1, nb_epochs=int(1e5),
    ...
    for epoch in tqdm(range(nb_epochs)):
        ...
        os.makedirs(f'novel_views/{epoch}', exist_ok=True)
        torch.save(nerf_model.state_dict(), f'novel_views/{epoch}/model.pth')

        for img_index in range(200):
            test(hn, hf, testing_dataset, img_index=img_index, nb_bins=nb_bins, H=H, W=W, epoch=epoch)
